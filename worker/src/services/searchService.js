...
import { withJson } from '../handlers/shared.js';
...
function hasValidSchemaVersion(value) {
  return /^\d+\.\d+\.\d+$/.test(String(value || ''));
}

export function validateSearchPayload(body, runtimeVersion = null) {
  const errors = [];
  ...
  if (body?.schemaVersion && !hasValidSchemaVersion(body.schemaVersion)) {
    errors.push('schemaVersion deve seguir o formato semver (x.y.z)');
  }

  if (body?.schemaVersion && runtimeVersion && String(body.schemaVersion) !== String(runtimeVersion)) {
    errors.push(`schemaVersion incompatível com runtime (${runtimeVersion})`);
  }
...
export async function executeSearch(rawPayload, env, requestId) {
  const cfg = getRuntimeConfig(env);
  const errors = validateSearchPayload(rawPayload, cfg.version);
  if (errors.length) {
    await incrementMetric(env, 'invalidPayloads', 1);
    return withJson({ error: 'Payload inválido', errors, schemaVersion: cfg.version }, 400, requestId);
  }
