import { LIMITS, SITES } from '../config.js';
...
export function tokensFromResume(text) {
  return String(text || '')
    .toLowerCase()
    .match(/[a-zà-ÿ][a-zà-ÿ0-9+#.-]{2,}/gi)?.slice(0, 12) || [];
}

export function buildLegacyQuery(body) {
...
export function normalizeExpectedSalaryRange(payload) {
...
export function parseSalaryValue(rawSalary) {
...
export function sortJobs(items, sortBy, sortDir) {
...
export function normalizeJobOutput(job) {
  return {
...
export async function processSearch(payload, env, rawPayload, cfg) {
